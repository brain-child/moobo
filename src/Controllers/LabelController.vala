public class LabelController {
    ...
    public LabelWidget movable { private set; get; }
    public LabelModel model { private set; get; }
    ...
    public LabelController (BoardController board_controller, int x, int y, LabelModel model = new LabelModel ()) {
        this.board_controller = board_controller;
        this.overlay = board_controller.overlay;
        this.movable = new LabelWidget (this);
        this.model = model;
        model.x = x;
        model.y = y;
    ...
    private bool on_key_press (Gdk.EventKey event) {
        ...
        if (key_name == "GDK_CONTROL_MASK" && (event.keyval == Gdk.Key.plus || event.keyval == Gdk.Key.equal)) {
    ...
    private void update_widget () {
        ...
        var rgba = Gdk.RGBA ();
        if (rgba.parse (model.color)) { change_color (rgba); }
    }

    public void change_color (Gdk.RGBA rgba) {
        var css_provider = new Gtk.CssProvider ();

        var css = CSS.printf (
            rgba.to_string (),
            Granite.contrasting_foreground_color (rgba).to_string ()
        );
        try {
        ...
        movable.textview.get_style_context ().add_provider (
            css_provider,
            Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION
        );
        movable.card.get_style_context ().add_provider (
            css_provider,
            Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION
        );

        movable.textview.get_style_context ().add_class ("color");
        movable.card.get_style_context ().add_class ("color");
    }
